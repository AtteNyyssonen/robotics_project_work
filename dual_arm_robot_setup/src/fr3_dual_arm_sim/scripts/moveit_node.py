...
class DualArmHoming(Node):
    def __init__(self):
        ...
        self.get_logger().info("Waiting 5 seconds for controllers and joint states to stabilize...")
        time.sleep(5.0) 
        self.get_logger().info("Simulation stabilization complete.")
    ...
    def plan_and_execute(self, arm_component, arm_name):
        ...
        if plan_result:
            ...
            execution_result = self.fr3.execute(plan_result, controllers=[])
            if execution_result:
